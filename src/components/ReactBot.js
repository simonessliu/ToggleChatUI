...
export default class ReactBot extends Component {
  ...
  async getWelcome() {
    // get welcome event
    const response = await axios.post('https://togglechat.simonlyllyyy.now.sh/dialogflow/event', { event: 'WELCOME' });
  ...
  async handleSendMessage(message) {
    ...
    const response = await axios.post('https://togglechat.simonlyllyyy.now.sh/dialogflow/query', { message });
    const { contents, richResponses } = response.data;
    this.appendMessage({ contents, richResponses, isUser: false});
  }
